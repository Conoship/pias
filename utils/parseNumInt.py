...
def _extract_conclusion(text: str):
    ...
    m_step = re.search(r"Numerical integration step accuracy.*?:\s*(\d+)", text, re.IGNORECASE)
...
def import_numint_rtf(
    ...
    if ship_name is None:
        raise ValueError("import_numint_rtf requires explicit ship_name/design_name/version/subversion (same as runInvoke usage).")
    ...
    for raw_line in text.splitlines():
        ...
        if line.lower().startswith("zero-compartment damages"):
            ...
            if len(nums) >= 7:
                cases.append((
                    "zero",
                    parse_float(nums[0]), parse_float(nums[1]),
                    parse_float(nums[2]), parse_float(nums[3]),
                    parse_float(nums[4]), parse_float(nums[5]),
                    parse_float(nums[6]),
                ))
            continue

        # ignore header / totals
        if line.startswith("Damage case") or "Tlight" in line or "Not-included probabilities" in line:
            continue
    ...
    length, req_r, att_a, complies, step_accuracy, penetration_reference = _extract_conclusion(text)
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("DELETE FROM numint_case WHERE ship_version_id = %s", (ship_version_id,))
            cur.execute("DELETE FROM numint_conclusion WHERE ship_version_id = %s", (ship_version_id,))
            ...
            if cases:
                cur.executemany(
                    ...
                    [
                        (
                            ship_version_id, side, damage_case,
                            pi_l, si_l, pi_p, si_p, pi_d, si_d, ai
                        )
                        for (damage_case, pi_l, si_l, pi_p, si_p, pi_d, si_d, ai) in cases
                    ],
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        print("If you REALLY want to run standalone, wire filename->ship_version parsing like parseProbdam does.")
