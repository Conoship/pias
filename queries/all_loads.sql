WITH comp_data AS (
    SELECT
        ...
        COUNT(DISTINCT comp.id) FILTER (WHERE comp.design_content_id_number = 3)   AS n_gas_oil,
        COUNT(DISTINCT comp.id) FILTER (WHERE comp.design_content_id_number = 4)   AS n_potable_water,
        COUNT(DISTINCT comp.id) FILTER (WHERE comp.design_content_id_number = 6)   AS n_ballast,
        COUNT(DISTINCT comp.id) FILTER (WHERE comp.design_content_id_number = 8)   AS n_void,
        COUNT(DISTINCT comp.id) FILTER (WHERE comp.design_content_id_number = 12)  AS n_cargohold_hatch
