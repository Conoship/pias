...
from backend.backend import run_agent_pipeline
...
def display_results(
    window: QWidget, prediction: float, lower: float, upper: float, R_compare_result: str
):
    """
    ...
    required_index_compare_label = results_widget.findChild(QLabel, "requiredIndexComparison")
...
def collect_user_defined_value(window: QWidget):
    ...
    pass_value = get_value_from_line_edit(window, "requiredIndexLineEdit", "Required Index R")
    if pass_value is None:
        return 1

    return (ship_name, subdivision_length, light_service_draft, subdivision_draft, light_gm_value, partial_gm_value, deep_gm_value, pass_value)
...
def handle_agent_pipeline(window: QWidget):
    ...
    if collect_file_paths(window):
        return
    main_dimensions_path, openings_path, internal_subdiv_path = collect_file_paths(window)

    # Check if user defined values exist.
    if collect_user_defined_value(window):
        return
    ship_name, subdivision_length, light_service_draft, subdivision_draft, light_gm_value, partial_gm_value, deep_gm_value, pass_value = collect_user_defined_value(window)
    
    value = get_value_from_line_edit(window, "requiredIndexLineEdit", "Required Index R")
    ...
    cur.execute = ("""INSERT INTO probdam_conclusion (subdivision_length) 
                   VALUE (%f);""", (subdivision_length))
    cur.execute = ("""INSERT INTO trim_gm  (condition_name, draft, gm) 
                   VALUE (%s, %f, %f);""", ("light", light_service_draft, light_gm_value))
    cur.execute = ("""INSERT INTO trim_gm  (condition_name, gm) 
                   VALUE (%s, %f);""", ("partial", partial_gm_value))
    cur.execute = ("""INSERT INTO trim_gm  (condition_name, gm) 
                   VALUE (%s, %f);""", ("deepest", deep_gm_value))
